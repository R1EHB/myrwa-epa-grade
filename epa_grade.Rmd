...
scale_fill_weather <- scale_fill_manual(values=c('Dry'='orangered',
                                                 'Wet'='deepskyblue'))
```
...
```{r load-precip}
# prcp <- read_csv(file = "~/Dropbox/Work/mystic/_share_db/Processed/Precip/logan-hourly/20150605/logan_hourly.csv",
#                  col_types=list(col_character(), col_double())) %>%
prcp <- read.csv(file = "~/Dropbox/Work/mystic/_share_db/Processed/Precip/logan-hourly/20150605/logan_hourly.csv", stringsAsFactors=FALSE) %>%
...
mutate(prcp, Year=year(Datetime)) %>%
  ...
  scale_fill_manual(values=c('Dry'='orangered', 'Wet'='deepskyblue')) +
...
# write_csv(loc, path="locations.csv")
```
...
df %>%
  ...
  geom_tile() +
  scale_fill_gradientn('No. Samples', colours=rev(scales::brewer_pal(type = "seq", palette = 'GnBu')(9))) +
  labs(x="Year", y="") +
...
df_2014_std_weather %>%
  ...
  scale_fill_manual('Standard', values=c("Swim"="deepskyblue", "Boat"="chartreuse3")) +
...
select(df, ID, Agency, ProjectID, WaterType, WaterBodyID, LocationID, 
  ...
  write.csv(file="bacteria_dataset.csv", row.names=FALSE)
filter(loc, LocationID %in% unique(df$LocationID)) %>%
  select(-Location_Waterbody) %>%
  write.csv(file="sample_locations.csv", row.names=FALSE)
```
